...
interface LocationProps {
  onLocationChange: (newLocation: { x: number; y: number; label: string; bounds: any; raw: any }) => void;
}

const SearchLocation = ({ provider, onLocationChange }: { provider: any, onLocationChange: any }) => {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    map.on('geosearch/showlocation', (event: GeoSearchResultEvent) => {
...
const Location: React.FC<LocationProps> = ({ onLocationChange }) => {
  ...
  return (
    <div>
      ...
      <MapContainer center={initialCenter} zoom={initialZoom} style={{ height: "500px", width: "100%" }}>
        <TileLayer
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        />
        <SearchLocation provider={new OpenStreetMapProvider()} onLocationChange={onLocationChange} />
      </MapContainer>
    </div>
  );
};
